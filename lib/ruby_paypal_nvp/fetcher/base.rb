  ...
  class BaseFetcher
    ...
    def call
      begin
        ...
        Raven.capture_message("Error processing #{self.class}"\
                              "for #{@subject}", extra: {
                                message: result
                              })
      end
    ...
    def load_api_response(options)
      res = Net::HTTP.post_form(URI(configuration['api_url']), options)
    ...
    def result_with_meta(account = nil, options = {})
    ...
    def request_options(options = {})
      {
        method: api_method,
        version: '204.0',
        user: RubyPaypalNvp.configuration.username,
